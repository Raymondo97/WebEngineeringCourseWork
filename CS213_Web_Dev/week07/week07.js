...
/* Compute the math of calculating monthly payment */
function computePayment(amount, apr, term) {
  let payment;
  let intRate = (apr / 100) / 12;
  let months = term * 12;

  payment = Math.pow((1 + intRate), -(months));
  payment = (amount * intRate) / (1 - payment);

  // Returns monthly payment as float
